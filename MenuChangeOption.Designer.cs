...
{
	public class MenuChangeOption : System.Web.UI.Page
	{
		...
		static MenuChangeOption()
		{
			MenuChangeOption.ci = new CultureInfo("en-US");
		}

		public MenuChangeOption()
		{
		}
		...
		private void CheckDrillDown()
		{
			...
			if (item == "month")
			{
				...
				this.TxtDayTo.Text = dateTime1.ToString("d/M/yyyy", MenuChangeOption.ci);
		...
		private void LoadReport()
		{
			...
			{
				dataTable = Business.MenuChangeOption(this.TxtDayFrom.Text, this.TxtDayTo.Text, null, null, null, null, null);
			}
			else if (this.PanelMonthly.Visible)
			{
				dataTable = Business.MenuChangeOption(null, null, this.LstMonthFrom.SelectedItem.Value, this.LstMonthTo.SelectedItem.Value, this.TxtYear.Text, null, null);
			}
			else if (this.PanelYearly.Visible)
			{
				dataTable = Business.MenuChangeOption(null, null, null, null, this.TxtYearFrom.Text, this.TxtYearTo.Text, null);
			}
		...
		private void ReportGrid_ItemCreated(object sender, DataGridItemEventArgs e)
		{
			...
			if (e.Item.DataItem is DataRowView)
			{
				...
				{
					item = new object[] { "<a href=\"MenuChangeOptionDetail.aspx?id=", row["MenuID"], "&yearfrom=", this.TxtYearFrom.Text, "&yearto=", this.TxtYearTo.Text, "\">" };
				...
				{
					item = new object[] { "<a href=\"MenuChangeOptionDetail.aspx?id=", row["MenuID"], "&monthfrom=", this.LstMonthFrom.SelectedItem.Value, "&monthto=", this.LstMonthTo.SelectedItem.Value, "&yearfrom=", this.TxtYear.Text, "\">" };
					stringBuilder.Append(string.Concat(item));
					stringBuilder.Append(row[0].ToString());
					stringBuilder.Append("</a>");
				}
				...
				{
					item = new object[] { "<a href=\"MenuChangeOptionDetail.aspx?id=", row["MenuID"], "&dayfrom=", this.TxtDayFrom.Text, "&dayto=", this.TxtDayTo.Text, "\">" };
					stringBuilder.Append(string.Concat(item));
					stringBuilder.Append(row[0].ToString());
					stringBuilder.Append("</a>");
				}
