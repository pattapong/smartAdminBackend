...
{
	public class MenuCancel : System.Web.UI.Page
	{
		...
		static MenuCancel()
		{
			MenuCancel.ci = new CultureInfo("en-US");
		}
		...
		private void CheckDrillDown()
		{
			...
			if (item == "month")
			{
				...
				this.TxtDayTo.Text = dateTime1.ToString("d/M/yyyy", MenuCancel.ci);
		...
		private void LoadReport()
		{
			...
			{
				dataTable = Business.MenuCancel(this.TxtDayFrom.Text, this.TxtDayTo.Text, null, null, null, null, null);
			}
			else if (this.PanelMonthly.Visible)
			{
				dataTable = Business.MenuCancel(null, null, this.LstMonthFrom.SelectedItem.Value, this.LstMonthTo.SelectedItem.Value, this.TxtYear.Text, null, null);
			}
			else if (this.PanelYearly.Visible)
			{
				dataTable = Business.MenuCancel(null, null, null, null, this.TxtYearFrom.Text, this.TxtYearTo.Text, null);
			}
		...
		private void ReportGrid_ItemCreated(object sender, DataGridItemEventArgs e)
		{
			...
			if (e.Item.DataItem is DataRowView)
			{
				...
				{
					item = new object[] { "<a href=\"MenuCancelDetail.aspx?id=", row["MenuTypeID"], "&yearfrom=", this.TxtYearFrom.Text, "&yearto=", this.TxtYearTo.Text, "\">" };
				...
				{
					item = new object[] { "<a href=\"MenuCancelDetail.aspx?id=", row["MenuTypeID"], "&monthfrom=", this.LstMonthFrom.SelectedItem.Value, "&monthto=", this.LstMonthTo.SelectedItem.Value, "&yearfrom=", this.TxtYear.Text, "\">" };
					stringBuilder.Append(string.Concat(item));
					stringBuilder.Append(row[0].ToString());
					stringBuilder.Append("</a>");
				}
				...
				{
					item = new object[] { "<a href=\"MenuCancelDetail.aspx?id=", row["MenuTypeID"], "&dayfrom=", this.TxtDayFrom.Text, "&dayto=", this.TxtDayTo.Text, "\">" };
					stringBuilder.Append(string.Concat(item));
					stringBuilder.Append(row[0].ToString());
					stringBuilder.Append("</a>");
				}
