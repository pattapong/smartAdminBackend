...
{
	public class ReserveTable : System.Web.UI.Page
	{
		...
		static ReserveTable()
		{
			ReserveTable.ci = new CultureInfo("en-US");
		}
		...
		private void CheckDrillDown()
		{
			...
			if (item == "month")
			{
				...
				this.TxtDayTo.Text = dateTime1.ToString("d/M/yyyy", ReserveTable.ci);
		...
		private void LoadReport()
		{
			...
			{
				dataTable = Business.ReserveTable(this.TxtDayFrom.Text, this.TxtDayTo.Text, null, null, null, null);
			}
			else if (this.PanelMonthly.Visible)
			{
				dataTable = Business.ReserveTable(null, null, this.LstMonthFrom.SelectedItem.Value, this.LstMonthTo.SelectedItem.Value, this.TxtYear.Text, null);
			}
			else if (this.PanelYearly.Visible)
			{
				dataTable = Business.ReserveTable(null, null, null, null, this.TxtYearFrom.Text, this.TxtYearTo.Text);
			}
		...
		private void ReportGrid_ItemCreated(object sender, DataGridItemEventArgs e)
		{
			if (e.Item.DataItem is DataRowView)
			{
				...
				{
					stringBuilder.Append(string.Concat("<a href=\"ReserveByPeriodDetail.aspx?yearin=", row[0].ToString(), "\">"));
					stringBuilder.Append(row[0].ToString());
					stringBuilder.Append("</a>");
				}
				else if (this.PanelMonthly.Visible)
				{
					string[] str = new string[] { "<a href=\"ReserveByPeriodDetail.aspx?monthin=", row[0].ToString(), "&yearin=", this.TxtYear.Text, "\">" };
				...
				{
					stringBuilder.Append(string.Concat("<a href=\"ReserveByPeriodDetail.aspx?datein=", row[0].ToString(), "\">"));
					stringBuilder.Append(row[0].ToString());
					stringBuilder.Append("</a>");
				}
