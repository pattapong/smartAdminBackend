...
{
	public class ServiceCheckInOut : System.Web.UI.Page
	{
		...
		static ServiceCheckInOut()
		{
			ServiceCheckInOut.ci = new CultureInfo("en-US");
		}

		public ServiceCheckInOut()
		{
		}
		...
		private void CheckDrillDown()
		{
			...
			if (item == "month")
			{
				...
				this.TxtDayTo.Text = dateTime1.ToString("d/M/yyyy", ServiceCheckInOut.ci);
		...
		private void LoadReport()
		{
			...
			{
				dataTable = Business.ServiceCheckInOut(this.TxtDayFrom.Text, this.TxtDayTo.Text, null, null, null, null, null);
			}
			else if (this.PanelMonthly.Visible)
			{
				dataTable = Business.ServiceCheckInOut(null, null, this.LstMonthFrom.SelectedItem.Value, this.LstMonthTo.SelectedItem.Value, this.TxtYear.Text, null, null);
			}
			else if (this.PanelYearly.Visible)
			{
				dataTable = Business.ServiceCheckInOut(null, null, null, null, this.TxtYearFrom.Text, this.TxtYearTo.Text, null);
			}
		...
		private void ReportGrid_ItemCreated(object sender, DataGridItemEventArgs e)
		{
			...
			if (e.Item.DataItem is DataRowView)
			{
				...
				{
					item = new object[] { "<a href=\"ServiceCheckInOutDetail.aspx?id=", row["WeekDay"], "&yearfrom=", this.TxtYearFrom.Text, "&yearto=", this.TxtYearTo.Text, "\">" };
				...
				{
					item = new object[] { "<a href=\"ServiceCheckInOutDetail.aspx?id=", row["WeekDay"], "&monthfrom=", this.LstMonthFrom.SelectedItem.Value, "&monthto=", this.LstMonthTo.SelectedItem.Value, "&yearfrom=", this.TxtYear.Text, "\">" };
					stringBuilder.Append(string.Concat(item));
					stringBuilder.Append(row[1].ToString());
					stringBuilder.Append("</a>");
				}
				...
				{
					item = new object[] { "<a href=\"ServiceCheckInOutDetail.aspx?id=", row["WeekDay"], "&dayfrom=", this.TxtDayFrom.Text, "&dayto=", this.TxtDayTo.Text, "\">" };
					stringBuilder.Append(string.Concat(item));
					stringBuilder.Append(row[1].ToString());
					stringBuilder.Append("</a>");
				}
