...
{
	public class SalesByEmp : System.Web.UI.Page
	{
		...
		static SalesByEmp()
		{
			SalesByEmp.ci = new CultureInfo("en-US");
		}
		...
		private void CheckDrillDown()
		{
			...
			if (item == "month")
			{
				...
				this.TxtDayTo.Text = dateTime1.ToString("d/M/yyyy", SalesByEmp.ci);
		...
		private void LoadReport()
		{
			...
			{
				dataTable = Business.SalesByEmp(this.TxtDayFrom.Text, this.TxtDayTo.Text, null, null, null, null);
			}
			else if (this.PanelMonthly.Visible)
			{
				dataTable = Business.SalesByEmp(null, null, this.LstMonthFrom.SelectedItem.Value, this.LstMonthTo.SelectedItem.Value, this.TxtYear.Text, null);
			}
			else if (this.PanelYearly.Visible)
			{
				dataTable = Business.SalesByEmp(null, null, null, null, this.TxtYearFrom.Text, this.TxtYearTo.Text);
			}
			...
			for (int i = 0; i < dataTable.Rows.Count; i++)
			{
				...
				for (int j = 3; j < dataTable.Columns.Count - 1; j++)
				{
			...
			if (dataTable.Rows.Count > 0)
			{
				...
				for (int k = 3; k < dataTable.Columns.Count; k++)
				{
